...
func (c *Config) updateDebrid(d Debrid) Debrid {
	// Default: 15 workers per provider. The former default (NumCPU * 50)
	// saturated CPU and RAM on home servers — a 4-core machine produced
	// 200 concurrent HTTP connections, causing OOM kills and CPU spikes.
	// Set workers explicitly in config.json if your server can handle more.
